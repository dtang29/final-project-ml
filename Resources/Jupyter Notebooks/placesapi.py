...
API_KEY = 'AIzaSyAT-aES724zLcK76imtVIqcg2L6M7OwJdk'
...
# for index, (address, latitude, longitude) in enumerate(zip(address.iloc[:,0], address.iloc[:,1], address.iloc[:,2])):
...
def get_google_results(business, address, latitude, longitude, api_key=None, return_full_response=False):
    ...
    if len(results['candidates']) == 0:
        ...
        output = {
            ...
            "price_level": answer.get("price_level")
        }
