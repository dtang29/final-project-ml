...
if __name__ == "__main__":
    for offset_num in np.arange(50,600,50) :
        try:
            output_json = search_grocers(offset_num)
            print(offset_num)
            with open('yelp_data/groc_'+str(offset_num)+'.json', 'w') as outfile:
    ...
    df_first.to_csv("yelp_data/output_api2.csv", index = False)
